...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        # features -> initial hidden state h0
        h0 = features @ W_proj + b_proj  # (N, H)

        # word embeddings for captions_in
        x_embed = word_embedding_forward(captions_in.long(), W_embed)  # (N, T, W)

        # recurrent forward over sequence -> hidden states h
        if self.cell_type == "rnn":
            h = rnn_forward(x_embed, h0, Wx, Wh, b)    # (N, T, H)
        else:  # 'lstm'
            h = lstm_forward(x_embed, h0, Wx, Wh, b)   # (N, T, H)

        # hidden -> vocab scores (temporal affine)
        scores = temporal_affine_forward(h, W_vocab, b_vocab)  # (N, T, V)

        # temporal softmax loss with mask over <NULL>
