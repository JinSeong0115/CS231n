...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_dot_product = torch.dot(z_i, z_j) / (torch.linalg.norm(z_i) * torch.linalg.norm(z_j))  # cosine similarity
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        # --- positive pairs: (k, k+N) and (k+N, k) ---
        pos_sim_1 = sim(z_k, z_k_N) / tau
        pos_sim_2 = sim(z_k_N, z_k) / tau
        ...
        for t in range(2 * N):
            if t == k:
                continue
            denom_1 += torch.exp(sim(z_k, out[t]) / tau)
        loss_k = -torch.log(torch.exp(pos_sim_1) / denom_1)
        ...
        for t in range(2 * N):
            if t == k + N:
                continue
            denom_2 += torch.exp(sim(z_k_N, out[t]) / tau)
        loss_kN = -torch.log(torch.exp(pos_sim_2) / denom_2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (out_left * out_right).sum(dim=1, keepdim=True) / (left_norm * right_norm + eps)  # (N,1)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return pos_pairs
...
def compute_sim_matrix(out):
    ...
    # (2N,2N): (out @ out^T) / (||out||·||out||^T)
    sim_matrix = (out @ out.t()) / (norms @ norms.t() + eps)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return sim_matrix
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    idx = torch.arange(2 * N, device=out.device)
    pos_idx = (idx + N) % (2 * N)
    
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = torch.exp(sim_matrix[idx, pos_idx].unsqueeze(1) / tau)  # (2N,1)
