...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # Text Embedding + Positional Encoding (N, T, W)
        tgt = self.embedding(captions)  # (N, T, W)
        tgt = self.positional_encoding(tgt)  # (N, T, W)

        # Project image features into a single token "memory" (N, 1, W)
        memory = self.visual_projection(features).unsqueeze(1)

        # Future token masking (causal mask): only allow i <= j  (T, T)
        tgt_mask = torch.tril(torch.ones(T, T, device=device, dtype=torch.bool))

        # Project onto word distribution after decoder pass
        dec_out = self.transformer(tgt=tgt, memory=memory, tgt_mask=tgt_mask)  # (N, T, W)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Average pool the patch embeddings to get a single feature vector per image
        pooled = torch.mean(encoded, dim=1)  # (N, embed_dim)

        # Project the pooled feature vector to the number of output classes
