...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pows = torch.pow(10000, -torch.arange(0, embed_dim, 2) / embed_dim)  # (D/2,)

        # Compute positional values sin/cos
        pe[0, :, 0::2] = torch.sin(i * pows)  # even
        pe[0, :, 1::2] = torch.cos(i * pows)  # odd
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        scores = Q @ K.transpose(2, 3) / self.scale

        # attn_mask: (T,S) -> (S,T)
        if attn_mask is not None:
            # Ensure small probabilities in softmax
            scores = scores.masked_fill(attn_mask.T == 0, float("-inf"))
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = (
            x.reshape(N, C, h_p, P, w_p, P)
             .permute(0, 2, 4, 1, 3, 5)
             .reshape(N, h_p * w_p, self.patch_dim)
        )
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None):
        ...
        short_cut = src
        attn_out = self.self_attn(query=src, key=src, value=src)  # (N, S, D)
        src = self.dropout_self(attn_out)
        src = src + short_cut
        src = self.norm_self(src)

        # Feed-Forward Network + Residual + LayerNorm
        short_cut = src
        ffn_out = self.ffn(src)  # (N, S, D)
        src = self.dropout_ffn(ffn_out)
        src = src + short_cut
