...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # input -> affine -> relu
        hidden_layer_output, cache_relu = affine_relu_forward(X, W1, b1)
        # affine -> output
        scores, cache_affine = affine_forward(hidden_layer_output, W2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        # regularization (0.5 is for convenience of derivation)
        loss += 0.5 * self.reg * ((np.sum(W1**2))+(np.sum(W2**2)))

        # gradient
        dhidden, dw2, db2 = affine_backward(dscores, cache_affine)  # output -> affine
        dx, dw1, db1 = affine_relu_backward(dhidden, cache_relu)  # relu -> affine -> input
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # From the 1st layer to the (L-1) layer
